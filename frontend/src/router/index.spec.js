...
function route(path, meta = {}, params = {}, query = {}) {
  return { path, fullPath: path, meta, params, query }
}
...
describe('router authGuard — 보호 라우트 가드', () => {
  ...
  it('OAuth callback route is public for unauthenticated users', () => {
    expect(authGuard(route('/oauth/callback'))).toBe(true)
  })
  ...
  it('workspace correction keeps query parameters', () => {
    useAuthStore().setTokens('a1', 'r1')
    const tripStore = useTripStore()
    tripStore.trips = [{ id: 10, status: 'planning' }]
    ...
    expect(
      authGuard(
        route(
          '/trips/10/log',
          { requiresAuth: true, workspace: 'record' },
          { tripId: '10' },
          { mode: 'itinerary' },
        ),
      ),
...
describe('router 라우트 등록 — 보호 누락 방지 (S2-CORE-02)', () => {
  ...
  const PUBLIC_PATHS = ['/', '/login', '/signup', '/forgot-password', '/reset-password', '/oauth/callback']
