...
//   - failed    : 일시적 실패(네트워크·서버) → 재시도 가능
//   - rejected  : 클라 검증 탈락(형식·크기) → 같은 파일 재시도는 무의미
//   - cancelled : 사용자가 취소
...
export function useUploadQueue(tripId, { concurrency = DEFAULT_CONCURRENCY, api } = {}) {
  ...
  const allDone = computed(() => items.length > 0 && items.every((it) => isTerminal(it.status)))
  const busy = computed(() =>
    items.some((it) => it.status === QueueStatus.UPLOADING || it.status === QueueStatus.LINKING),
  )
  const failedCount = computed(
    () => items.filter((it) => it.status === QueueStatus.FAILED).length,
  )
  const linkedCount = computed(
    () => items.filter((it) => it.status === QueueStatus.LINKED).length,
  )
  ...
  async function process(item) {
    try {
      ...
      if (item.status === QueueStatus.CANCELLED) return // 취소는 실패로 치지 않는다.
  ...
  // 실패 항목만 재시도. 진행 중·완료·검증탈락은 무시(중복 트리거 방지).
  function retry(id) {
  ...
  // 스코프(컴포넌트) 해제 시 모든 objectURL 해제 — 메모리 누수 방지.
  onScopeDispose(() => items.forEach(cleanup))

  return { items, addFiles, retry, remove, cancel, allDone, busy, failedCount, linkedCount }
}

function isTerminal(status) {
  // FAILED 는 재시도 가능하므로 '완료'로 보지 않는다.
  return (
    status === QueueStatus.LINKED ||
