...
- [ ] **[S3-TRIP-03]** 구간 소요시간 표시 — 자동차 = 카카오모빌리티 길찾기 API 연동 시도(#39 공통 인프라 활용), **미연동·실패 시 추정/수동 폴백이 P0 합격선(외부 API는 P0 blocker 아님)** / 도보·대중교통 = 추정·수동 입력. Notes: TMAP(보행)·ODsay(대중교통) 최신 제공 여부 확인. 실이동경로 = P1, 실시간 소요시간 = P2
...
- [ ] **[S3-LOG-02]** 외곽선 사이드카 도입 — `outline_module`/`serve_outline` 제품 포팅, 업로드 1회 전처리 = **비동기 큐(업로드 차단 금지)** (0006 D1·D2 — SAM 인코딩 캐시 검토 + 처리 시간/RSS 측정 기준 포함)
- [ ] **[S3-LOG-03]** 텍스트 LLM 어댑터 + 프롬프트 — 객체별 **짧은 문구만**, 배치 = OUTLINE_API §2-1 기반 (S3-CORE-02 위에서). **역할 경계: 좌표·앵커 후보 = 코드(사이드카) 산출, LLM = 후보 중 선택 + 문구 생성만 — 임의 좌표 생성 금지(0004 D3 유지)**
...
- **트랙 결정 요약 (서로 검토)**:
  ...
  - (trip) 구간 소요시간 = TMAP 채택(자동차·도보), 대중교통 = TMAP 일 10회 제한으로 수동 입력. 후속 #96·#97·#98 = Codex가 사람 합의 없이 발행 → 팀원이 닫음(미채택).
