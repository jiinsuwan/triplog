...
4. **PR 후 전환** — 로컬 테스트 통과 → **사람에게 발행 확인을 받고 PR 생성**. 작업이 끝났으면 **PR 발행 단계를 건너뛰지 않는다** — 코멘트·요약 보고로 갈음하지 않는다 (S2 회고: 발행 누락 반복으로 명문화). 발행 시 **상대를 reviewer로 지정**한다 — 지정해야 상대에게 GitHub 알림이 간다. 리뷰는 비동기이므로 **이 브랜치에는 더 쌓지 않습니다.** 리뷰를 기다리는 동안 CI · 리뷰 코멘트에 대응하거나, **독립적인 다른 Issue를 새 브랜치에서 시작**합니다. 한 브랜치에 여러 Issue를 합치지 않습니다.
5. **머지** — **모든 PR은 상대 리뷰(Approve) 후 머지**합니다 (§1-3 상호 리뷰 — S2 회고에서 self-merge 폐지).
...
3. **기계 검사** — 객관적으로 검사 가능한 규칙(본문 필수 섹션, 공유 경로 접촉, 보호 라우트 등)은 **CI로 강제**한다 — 리뷰 운에 맡기지 않는다. PR guardrail(#59 채택): 본문 검사 = feat·fix PR required check / 공유 경로 = 자동 코멘트 + `track:core` 라벨. 구현은 S3 core Issue.
...
- **Summary** — 변경 요약 (무엇을 · 왜)
- **Agent 작업** — 에이전틱 협업 로그 (입력 / 변경 / 사람이 검토한 것)
- **테스트 증거** — 로컬 테스트 · CI · 화면/API 확인
- **공유 영역 영향** — §6-3 해당 여부와 내용
- **잔여 리스크 / 후속**

**템플릿 헤더 문구는 고정한다** — 번역·변형(예: `## Summary` → `## 요약`) 금지. 본문 검사 CI가 헤더 문자열 기준으로 검사하므로 변형하면 내용이 있어도 누락으로 판정된다 (S2 회고).
...
머지는 트랙 구분 없이 상대 리뷰를 거칩니다(§1-3). 위 목록은 **특히 주의 깊은 리뷰가 필요한 영역**이며, 해당 시 Issue·PR 본문에 명시할 의무가 있습니다.
