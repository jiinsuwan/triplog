...
> **이 PoC가 통과해야 P0 카드 생성 기능이 성립한다.** 실패 시 §7 폴백 시나리오 중 하나로 전환한다 — 본격 개발 일정에는 영향 없음.
...
**PoC 실패 시**: 폴백 시나리오(§7) 중 하나로 전환. 본격 개발 일정에는 영향 없음.
...
요청: 인스타 피드용 카드 (1080×1080) 레이아웃 JSON을 만들어주세요.
포함할 요소:
- 메인 캡션 (손글씨 톤, 1~2줄)
- 객체를 가리키는 화살표 1~2개
- 강조할 단어의 물결 언더라인
...
좌표는 1080×1080 기준 픽셀값으로.
```

#### 출력 JSON 스키마
```json
{
  "canvas": { "width": 1080, "height": 1080, "background": "#1a1410" },
...
| Export 정확도 | 1080×1080 PNG, 폰트 안 깨짐 |
