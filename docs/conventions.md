...
- **진행상황의 단일 출처(SSOT) = GitHub Issue/PR 상태**입니다. 스프린트 문서의 체크박스는 **계획 시 작성하고, 스프린트 종료(회고) 때 한 번 맞춥니다** — 작업마다 실시간 갱신하지 않습니다(두 곳을 동시에 관리하면 어긋남). "지금 무엇이 끝났나"는 Issue/PR로 봅니다.
