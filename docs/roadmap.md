...
### 둘이 함께 (core)
- [x] **[S0-CORE-01]** Repo 생성 ✓ / collaborator 초대(사람 확인) / branch protection: 당시 플랜 제약으로 미적용 → 컨벤션 운용 (**이후 main 리뷰 필수 보호 활성화됨** — 현행 규칙 = conventions §1-3)
...
- [ ] **[S3-TRIP-01]** 일정(stop) 스키마 + CRUD API — 날짜·시간·장소·순서 + **이동수단(필수 필드)**, 사진 선택적 연결 고려 — F04 데이터 계층. **스키마 = core 리뷰 필수** (S2 회고)
- [ ] **[S3-TRIP-02]** 일정 편집 화면 — 날짜별 장소 추가·드래그앤드롭 정렬 + 지도 핀 순서 동선 — F04 화면
- [ ] **[S3-TRIP-03]** 구간 소요시간 표시 — 자동차 = 카카오모빌리티 길찾기 API 연동 시도(S2 외부 API 인프라 활용), **미연동·실패 시 추정/수동 폴백 = P0 합격선(외부 API는 P0 blocker 아님)**, 도보·대중교통 = 추정/수동 (P1 실이동경로 / P2 실시간 소요시간)
...
### core 트랙 (공통)
- [ ] **[S3-CORE-01]** PR guardrail (#65) — 본문 검사 CI(feat·fix = required) + 공유 경로 감지(자동 코멘트 + `track:core` 라벨) + reviewer 미지정 경고 (**#59 채택**) — S2 회고
- [ ] **[S3-CORE-02]** AI 호출 공통 인프라 (#66) — `LlmAdapter`, provider 1개=GMS 우선, AiCallLog — **Sprint 2에서 이동**
- [ ] **[S3-CORE-03]** IA 라우팅 + status 체계 (#67) — 계획/기록 2워크스페이스·상태 기반 진입([frontend-structure §3-0](frontend-structure.md)), status 허용값·검증, 목록 응답 size (**#17 잔여 흡수 → #17 닫기**) — S2 회고
- [ ] **[S3-CORE-04]** 비밀번호 찾기 (#68) — 이메일 인프라 없는 방식 우선 확인 (성격 = **심사기준 보강, P0 아님** — P0 지장 시 S4 이월, S2 회고)
- (번호 미부여 · 재검토) 사진/카드 서빙·권한 검증 보강 — S2에서 인증 엔드포인트 서빙으로 선반영(#38), 잔여 범위 재검토 후 필요 시 이슈화·번호 부여
...
| v1.2.1 | 2026-06-12 | **S3 core 번호 재정렬** (발행 이슈 기준 01부터 연속, 빈 슬롯 제거): 01 = guardrail(#65) / 02 = AI 인프라(#66) / 03 = IA·status(#67) / 04 = 비밀번호 찾기(#68). 서빙 보강 재검토 항목은 번호 미부여로 강등 |
