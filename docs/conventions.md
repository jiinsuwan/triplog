...
4. **PR 후 정지** — 로컬 테스트 통과 → PR 생성 → **멈춥니다.** 사람 리뷰 · 머지 전에 다음 Issue로 넘어가지 않습니다.
5. **머지** — 트랙 내부 변경은 self-merge(CI green 후), core 변경은 상대 리뷰 후 머지(§6-3).

의존성이 있는 Issue(예: core 머지 후 결합)는 앞 Issue가 머지된 뒤 다음 세션에서 진행합니다.
