...
- [ ] **[S1-CORE-04]** 로그인·회원가입 화면 + 라우트 가드 (#21) — 프론트 인증 동작 (종료조건① 보완)
...
### trip 트랙
- [ ] **[S2-TRIP-01]** 관광지 표준데이터 JSON 적재 (Flyway seed 또는 적재 스크립트) — Q3. **데모 지역 일부부터**(전국 X)
- [ ] **[S2-TRIP-02]** 카카오맵 SDK 연동 + 장소 검색·지도 탐색 화면(식당·카페 포함) + 마커 — F03. **(이월 #28 장소/지도 화면 흡수 — 목업→실 SDK 재작성)**
...
- [ ] **[S2-CORE-02]** 라우트 보호 `requiresAuth` 자동 테스트 (회고 B-2 채택 — conventions §6-3)
