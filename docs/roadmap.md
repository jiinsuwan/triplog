...
- **매 Sprint 두 사람 모두 자기 파트 작업**이 있다 (한쪽만 일하는 구간 없음).
  - `trip`: 여행 → 관광지 → 일정 → 챗봇·담은 장소(F10 흡수)
...
Sprint 3  ─ 일정 에디터 & 카드 생성  → P0 완성     ┘
   │
Sprint 4  ─ P1 추가 (소셜로그인·담은 장소 F10 흡수·챗봇 / 사진 라이브러리·카드 편집·정사각)
```
...
### trip 트랙
- [ ] **[S4-TRIP-01 / GitHub #112]** F10 즐겨찾기/위시리스트 처분 — 별도 CRUD를 구현하지 않고, 사용자 역할이 중복되는 기존 여행별 `담은 장소`(Pocket)를 후보 장소 보관 흐름으로 사용. 별도 전역 마이페이지 즐겨찾기는 현재 범위가 아니며, 담은 장소 UX 개선은 별도 Issue로 분리
