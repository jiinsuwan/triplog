...
class Engine:
    ...
    def __init__(self, morph=7, off_ratio=0.006, snap=False, tight=True, use_cache=True):
        self.morph = morph
        self.off_ratio = off_ratio
        self.snap = snap
        self.tight = tight
        self.use_cache = use_cache   # SAM 인코딩 캐시: set_image 1회 + 프롬프트 N회 (A1 스파이크)
        self._det = None
        self._sam = None
        self._sal = None
        self._sam_pred = None        # 캐시 경로용 SAM2Predictor (인코딩-디코딩 분리)
        self._feat_cache = {}        # id(img) -> (img_ref, features) — 이미지별 인코딩 캐시
    ...
    def sam_predictor(self):
        if self._sam_pred is None:
            from ultralytics.models.sam import SAM2Predictor
            self._sam_pred = SAM2Predictor(overrides=dict(
                task='segment', mode='predict', imgsz=1024,
                model='weights/sam2.1_b.pt', save=False, verbose=False))
        return self._sam_pred

    def sam_prompt(self, img_s, points=None, labels=None, bboxes=None):
        """SAM 프롬프트 1회. use_cache면 이미지 인코딩을 캐시하고 디코딩만 수행.
        같은 배열 객체(is 비교) = 같은 이미지 — 서버는 store에 img_s를 보관하므로 성립."""
        if not self.use_cache:
            return self.sam(img_s, points=points, labels=labels, bboxes=bboxes, verbose=False)[0]
        pred = self.sam_predictor
        ent = self._feat_cache.get(id(img_s))
        if ent is None or ent[0] is not img_s:
            import torch
            with torch.inference_mode():           # set_image는 정규 경로와 달리 autograd가
                pred.set_image(img_s)              # 켜진 채 돌아 2배 느림 — 명시적으로 차단 (A6)
            self._feat_cache[id(img_s)] = (img_s, pred.features)
            if len(self._feat_cache) > 4:          # 단순 FIFO 퇴출 (4장 ≈ 임베딩 70MB)
                self._feat_cache.pop(next(iter(self._feat_cache)))
        elif pred.features is not ent[1]:
            pred.setup_source(img_s)               # 이미지 전환: 소스 교체(저가) + 인코딩 복원
            pred.features = ent[1]
        return pred(points=points, labels=labels, bboxes=bboxes)[0]
        ...
        for c in carriers[:2]:
            ...
            for px, py in pts[:16]:
                ...
                if any(pv[int(py), int(px)] > 127 for pv in grid_masks):
                    continue
                res_g = self.sam_prompt(img_s, points=[[px, py]], labels=[1])
    ...
    def sam_tight(self, img_s, boxes):
        ...
        Hs, Ws = img_s.shape[:2]
        res = self.sam_prompt(img_s, bboxes=boxes)
        ...
        if refined_boxes:
            res2 = self.sam_prompt(img_s, bboxes=refined_boxes)
    ...
    def outline_at(self, img_s, pt_norm):
        ...
        res = self.sam_prompt(img_s, points=[[float(px), float(py)]], labels=[1])
        masks = self._to_masks(res, (Hs, Ws))
        if not masks:
            return None
        return {'loops': self.mask_to_loops(masks[0], img_s), 'mask': masks[0]}

    def outline_refine(self, img_s, pos_norm, neg_norm=()):
        """+/− 클릭 정제 (A8): 한 객체에 포지티브/네거티브 점 묶음 1회 프롬프트.
        v10 탭의 일반화 — pos 1개 + neg 0개면 outline_at과 동일. 어두운 림 등
        과포함/누락을 사람이 점 2~3개로 다듬는 용도."""
        Hs, Ws = img_s.shape[:2]
        pts = [[float(x * Ws), float(y * Hs)] for x, y in list(pos_norm) + list(neg_norm)]
        labels = [1] * len(pos_norm) + [0] * len(neg_norm)
        if not pts:
            return None
        # (1, num_points, 2) + (1, num_points) = "객체 1개, 점 여러 개"
        res = self.sam_prompt(img_s, points=[pts], labels=[labels])
        masks = self._to_masks(res, (Hs, Ws))
        if not masks:
            return None
        return {'loops': self.mask_to_loops(masks[0], img_s), 'mask': masks[0]}
    ...
    def outline_box(self, img_s, box_norm):
        ...
        res = self.sam_prompt(img_s, bboxes=[[x1, y1, x2, y2]])
        ...
        if cov >= 0.45:
            ...
            for px in gx:
                for py in gy:
                    res_p = self.sam_prompt(img_s, points=[[float(px), float(py)]], labels=[1])
